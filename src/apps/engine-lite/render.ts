...
function imageMarkup(post: LitePost, className: string, eager = false) {
...
function cardMarkup(post: LitePost, href: string) {
	return `<a class="lite-card-link" href="${escapeHTML(href)}">
		${imageMarkup(post, 'lite-card-image')}
...
function featuredMarkup(post: LitePost, href: string) {
	return `<section class="lite-featured">
		<span class="lite-featured-label">Pinned</span>
		<a class="lite-featured-link${post.image ? '' : ' has-no-image'}" href="${escapeHTML(href)}">
			${imageMarkup(post, 'lite-featured-image', true)}
...
export function renderFeed(portal: LitePortal, filters: FeedFilters, postHref: (post: LitePost) => string) {
	...
	if (!pinned) return '<div class="lite-empty">No published posts yet.</div>';
	...
	const posts = portal.posts.filter((post) => {
		if (post.id === pinned.id) return false;
	...
	return `<main class="lite-feed">
		${featuredMarkup(pinned, postHref(pinned))}
		...
		${
			posts.length
				? `<div class="lite-grid">${posts.map((post) => cardMarkup(post, postHref(post))).join('')}</div>`
