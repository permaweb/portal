  ...
  return (
    <S.CategorySpotlight>
      ...
      <S.CategorySpotlightGrid>
        <S.Left>
        {Object.values(Posts || [0,1,2,3,4,5])?.slice(0, 6).map((post: any, index: number) => (
          <LeftRow post={post} key={index} index={index} />
        ))}
        </S.Left>
        <S.Right>
        {Object.values(Posts || [0,1,2,3,4,5,6,7,8])?.slice(6, 9).map((post: any, index: number) => (
